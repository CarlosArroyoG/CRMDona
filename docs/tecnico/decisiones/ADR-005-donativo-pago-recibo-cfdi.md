...
| `Cfdi` → `ExternalCfdi` | Comprobante fiscal deducible | 3 / ADR-012 | **Actualizado 2026-09-23 (ADR-012):** el CRM no emite CFDI. La contadora los emite fuera; el CRM guarda el XML/PDF externo como antecedente (`external_cfdis`) y avisa a Contabilidad (`accounting_notices`). `Cfdi` queda como historial de solo lectura |
